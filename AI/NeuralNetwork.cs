...
public class NeuralNetwork
{
    ...
    /// Constructor.
    /// </summary>
    /// <param name="_id">Unique ID of the neural network.</param>
...
#pragma warning restore CS8618
    {
        ...
        if (layerDefinition.Length < 2) throw new ArgumentException("A minimum of 2 layers are required.", nameof(layerDefinition));
        ...
        while (!mutated) // ensure SOMETHING changes, otherwise we'll get two identical cars.
        {
            for (int layerIndex = 0; layerIndex < Biases.Length; layerIndex++)
            {
                for (int neuronIndex = 0; neuronIndex < Biases[layerIndex].Length; neuronIndex++)
                {
                    ...
                    {
                        mutated = true;
                        Biases[layerIndex][neuronIndex] += (float)RandomNumberGenerator.GetInt32((int)(-val * 100000), (int)(val * 100000)) / 200000;
                    }
                }
            }
            ...
            for (int layerIndex = 0; layerIndex < Weights.Length; layerIndex++)
            {
                for (int neuronIndexForLayer = 0; neuronIndexForLayer < Weights[layerIndex].Length; neuronIndexForLayer++)
                {
                    for (int neuronIndexInPreviousLayer = 0; neuronIndexInPreviousLayer < Weights[layerIndex][neuronIndexForLayer].Length; neuronIndexInPreviousLayer++)
                    {
                        ...
                        {
                            mutated = true;
                            Weights[layerIndex][neuronIndexForLayer][neuronIndexInPreviousLayer] += (float)RandomNumberGenerator.GetInt32((int)(-val * 100000), (int)(val * 100000)) / 200000;
                        }
                    }
                }
            }
    ...
    internal static void SaveTrainedModel()
    {
        ...
        foreach (int id in s_networks.Keys)
        {
            ...
            nn.Save(Path.Combine(/*Program.aiFolder*/@"c:\temp\", $"CarDodge{nn.Id}.ai"));
        }
    ...
    internal static void LoadTrainedModel()
    {
        ...
        {
            NeuralNetwork nn = s_networks[id];
            nn.Load(Path.Combine(/*Program.aiFolder*/@"c:\temp\", $"CarDodge{nn.Id}.ai"));
        }
